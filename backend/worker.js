...
async function buscarRecomendacao(env, corpo) {
  ...
  const plataforma = listaINT(corpo.plataforma);
  ...
  if (plataforma.length === 0) { throw new ErroValidacao('O campo "plataforma" é obrigatório (número inteiro).'); }
...
function condicoesBase({
  ...
  condicoes.push(`platforms = (${plataforma.join(',')})`);
...
// No momento não esta sendo usada para nada
