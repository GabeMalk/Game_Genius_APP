...
//   "eras": ["atualidade"],             // opcional
//   "historicoIds": [1022, 341705]      // opcional — últimos jogos sorteados
// }
//
...
// Filtrando para criar o dicionario dos filtros basicos a partir do dicionario maior. Esse filter basicamente é um for loop que retorna tudo que for
...
// Criando os campos que vamos pedir ao IGDB (Apicalypse) e juntamos eles com virgula
const CAMPOS = [
  'name',
  'summary',
  'cover.url',
...
class ErroValidacao extends Error {} // Perguntar
...
// ============================================================
// Funções de "tradução" para Apicalypse
// ============================================================
...
// Questões filosoficas: É ESTRITAMENTE necessario essa brisa de traduzir?? Poderia eu apenas
// fazer a const de filtros já traduzida? Poderia. Seria mais fácil? Seria. Seria melhor?
// .... meh, provavelmente não. Do jeito que esta agora é mais complexo, porém mais "elegante", e tem
// mais camadas, lida melhor com mudanças na API, torna a const dos filtros mais legivel, permite outras propriedade (basico)
// É mais trabalhoso, mas é _melhor_.
...
// ============================================================
// Querry
// Basicamente a função que vai montar a querry em Apicalypse para o operador
// ============================================================
...
function montarQuery({ baseParams, filtros, modo, comQualidade, offset }) {
  ...
  const where = condicoes.join(' & ');
  return `fields ${CAMPOS}; where ${where}; sort rating_count desc; limit 500; offset ${offset};`;
}

// Parece que a ordem desses topicos e funções ta bem maluca né...
...
// ============================================================
// MATCH LOCAL (Branch 2)
// Verifica se um jogo satisfaz um filtro específico (olhando os
// ids reais que a IGDB devolveu pra aquele jogo), e conta quantos
// dos filtros selecionados ele satisfaz no total.
// ============================================================
function jogoSatisfazFiltro(jogo, chave) {
  const def = FILTROS_CATALOGO[chave];
  if (!def) return false;

  const idsGenero = new Set((jogo.genres ?? []).map((g) => g.id));
  const idsTema = new Set((jogo.themes ?? []).map((t) => t.id));
  const idsKeyword = new Set((jogo.keywords ?? []).map((k) => k.id));
  const idsPerspectiva = new Set((jogo.player_perspectives ?? []).map((p) => p.id));

  if (def.generos?.some((id) => idsGenero.has(id))) return true;
  if (def.temas?.some((id) => idsTema.has(id))) return true;
  if (def.keywords?.some((id) => idsKeyword.has(id))) return true;
  if (def.perspectivas?.some((id) => idsPerspectiva.has(id))) return true;
  return false;
}

function contarFiltrosSatisfeitos(jogo, chaves) {
  return chaves.filter((chave) => jogoSatisfazFiltro(jogo, chave)).length;
}

function escolherComMatchMinimo(jogos, chaves, minimo) {
  const comMatchSuficiente = jogos.filter((j) => contarFiltrosSatisfeitos(j, chaves) >= minimo);
...
// ============================================================
// ORQUESTRADOR — implementa os branches do doc, na ordem
// ============================================================
async function buscarRecomendacao(env, corpo) {
  const plataforma = INTorNULL(corpo.plataforma);
  if (plataforma === null) {
    throw new ErroValidacao('O campo "plataforma" é obrigatório (número inteiro).');
  }

  const eras = ChecarEras(corpo.eras);
  const filtrosNegativos = ChecarFiltrosValidos(corpo.filtrosNegativos);
  ...
  if (filtros.length === 0) {
    // "Não existe aleatório": escolhe em segredo 1 filtro básico.
    // Isso garante que sempre caímos no Branch 1 (length === 1).
    filtros = [FiltroBasicoAleatorio()];
  }
  ...
  if (filtros.length <= 3) {
    ...
    if (pagina1.length === 500) {
      const pagina2 = await buscarPagina(
        env,
    ...
    const semQualidade = await buscarPagina(
      env,
    ...
    // 1.3.1 (fail): cai pro Branch 2 abaixo.
  }

  // ---------------- BRANCH 2: >3 filtros, ou Branch 1 esgotado ----------------
  const pagina1b2 = await buscarPagina(
    env,
    montarQuery({ baseParams, filtros, modo: 'OR', comQualidade: true, offset: 0 }),
  );

  if (pagina1b2.length === 500) {
    const pagina2b2 = await buscarPagina(
      env,
      montarQuery({ baseParams, filtros, modo: 'OR', comQualidade: true, offset: 500 }),
    );
    return escolherComMatchMinimo(pagina1b2.concat(pagina2b2), filtros, 2); // 2.1
  }
  if (pagina1b2.length >= 1) {
    return escolherComMatchMinimo(pagina1b2, filtros, 2); // 2.2
  }

  const semQualidadeB2 = await buscarPagina(
    env,
    montarQuery({ baseParams, filtros, modo: 'OR', comQualidade: false, offset: 0 }),
  );
  if (semQualidadeB2.length >= 1) {
    return escolherComMatchMinimo(semQualidadeB2, filtros, 2); // 2.3 sucesso
  }

  return null; // 2.3.1 (fail): nada encontrado
}
...
function melhorarImagemDoJogo(jogo) {
  ...
  return {
    ...
    cover: {
      ...jogo.cover,
      url: jogo.cover.url.replace('t_thumb', 't_cover_big').replace('//', 'https://'),
    },
  };
}
